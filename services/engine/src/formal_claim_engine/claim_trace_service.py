...
def _heuristic_document_extraction(
    *,
    ...
    def _heuristic_relation(sentence: str) -> tuple[str, str]:
        lowered = sentence.lower()
        challenge_cues = (
            "however",
            "but ",
            ...
            "depends on a critical assumption",
        )
        statistical_cues = (
            "%", "study", "survey", "benchmark", "analysis", "employees",
            "months", "tracked", "found", "2022", "2023",
        )
        if any(cue in lowered for cue in challenge_cues):
            return "weakens", "abductive"
        if any(cue in lowered for cue in statistical_cues):
            return "supports", "statistical"
        return "supports", "abductive"
    for index, (sentence, start_offset) in enumerate(units[:max_units]):
        ...
        if index == 0:
            continue
        relation_type, strength = _heuristic_relation(sentence)
        relations.append(
            {
                ...
                "strength": strength,
                "rationale": "Deterministic sentence-order fallback relation chain.",
            }
        )
    return {
        "claims": claims,
...
def _normalize_trace_strength(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        aliases = {
            ...
            "probabilistic": Strength.statistical.value,
            "statistic": Strength.statistical.value,
            "witness": Strength.testimonial.value,
        }

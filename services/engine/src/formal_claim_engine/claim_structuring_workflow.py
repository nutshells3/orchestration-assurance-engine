...
def empty_candidate_ledger() -> dict[str, list[dict[str, Any]]]:
    return {
        "claims_proposed": [],
        "claims_accepted": [],
        "claims_rejected": [],
...
class ClaimStructuringWorkflowState(BaseModel):
    ...
    candidate_ledger: dict[str, list[dict[str, Any]]] = Field(
        default_factory=empty_candidate_ledger
    )
    ...
    def merge_candidate_ledger(
        self,
        candidate_ledger: dict[str, list[dict[str, Any]]] | None,
    ) -> None:
        if not candidate_ledger:
            return
        for bucket, entries in candidate_ledger.items():
            self.candidate_ledger.setdefault(bucket, [])
            self.candidate_ledger[bucket].extend(list(entries or []))
